		...
		for(Biome biome : ForgeRegistries.BIOMES)
		{
			if(HunterConfig.hunter_weight.get() > 0) {
				...
				if(isMobInputBiomeOrCategory(biome, HunterConfig.spawn_biomes.get(), hunter)) {
					biome.getSpawns(hunter.getClassification()).add(new SpawnListEntry(HunterModEntityTypes.HUNTER_ENTITY, HunterConfig.hunter_weight.get(), HunterConfig.hunter_min_group.get(), HunterConfig.hunter_max_group.get()));
